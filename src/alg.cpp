...
// Факториал (потребуется для getPerm2)
static std::size_t factorial(std::size_t m) {
  std::size_t f = 1;
  for (std::size_t i = 2; i <= m; ++i) f *= i;
  return f;
}

// Все перестановки (обход дерева)
...
// Медленный метод – через getAllPerms
...
// Быстрый метод – навигация по дереву
